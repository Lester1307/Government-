...
async function callWithRetry(fn, retries = 2, delay = 1e3) {
  try {
    return await fn();
  } catch (err) {
    if (retries <= 0) {
...
function getLocalAdvisorFallback(advisorId, gameState) {
  const nameMap = {
...
function getLocalPolicyFallback(policyText, gameState) {
        ...
        () => ai.models.generateContent({
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                policyName: { type: import_genai.Type.STRING, description: "Official, elegant title of the policy in English/Hindi transliterated (e.g. 'Pradhan Mantri High-Speed Rail Mission')" },
                description: { type: import_genai.Type.STRING, description: "Narrative summary of the outcome, media headlines, and overall sentiment." },
                fiscalCost: { type: import_genai.Type.NUMBER, description: "Cost in Lakh Crores of Rupees. Spending is positive, revenue generation is negative." },
                gdpGrowthImpact: { type: import_genai.Type.NUMBER, description: "Effect on GDP Growth Rate in percentage points (e.g., 0.3 for +0.3%, -0.1 for -0.1%)." },
                inflationImpact: { type: import_genai.Type.NUMBER, description: "Effect on Inflation rate in percentage points (e.g., 0.15 for +0.15%)." },
                unemploymentImpact: { type: import_genai.Type.NUMBER, description: "Effect on Unemployment rate in percentage points (e.g., -0.2 for -0.2%)." },
                popularityImpact: { type: import_genai.Type.NUMBER, description: "Overall general approval effect (points, e.g. 4.5 or -3.0)." },
                demographicsImpact: {
                  type: import_genai.Type.OBJECT,
                  required: ["farmers", "youth", "corporate", "middleClass", "rural", "urban"],
                  properties: {
                    farmers: { type: import_genai.Type.NUMBER, description: "Impact on Farmer approval (points)" },
                    youth: { type: import_genai.Type.NUMBER, description: "Impact on Youth approval (points)" },
                    corporate: { type: import_genai.Type.NUMBER, description: "Impact on Corporate approval (points)" },
                    middleClass: { type: import_genai.Type.NUMBER, description: "Impact on Middle Class approval (points)" },
                    rural: { type: import_genai.Type.NUMBER, description: "Impact on Rural approval (points)" },
                    urban: { type: import_genai.Type.NUMBER, description: "Impact on Urban approval (points)" }
                  }
                },
                advisorReactions: {
                  ...
                  items: {
                    ...
                    properties: {
                      advisorId: { type: import_genai.Type.STRING, description: "One of: nirmala, amit, jaishankar, rajnath, gadkari, piyush, dharmendra, shivraj, nadda, bhupendra, somnath" },
                      name: { type: import_genai.Type.STRING, description: "Display name e.g. Nirmala Shastry" },
                      quote: { type: import_genai.Type.STRING, description: "Advisor's direct quote reacting to the policy." }
                    }
                  }
                }
              }
